...
function buildFixedPairCycleShortcutOptions(pairTeams, players, maxMatches = 0) {
  const validation = validateFixedPairTeams(pairTeams, players);
  const roundRobinMatches = calcFixedPairRoundRobinMatches(validation.validTeamsCount);
  if (roundRobinMatches < 1) return [];

  const cap = Math.max(0, Math.floor(Number(maxMatches) || 0));
  return FIXED_PAIR_CYCLE_SHORTCUTS.map((cycle) => {
    const value = roundRobinMatches * cycle;
    return {
      key: `cycle_${cycle}`,
      value,
      cycle,
...
function buildFixedPairShortcutHint() {
  return '固搭循环赛支持多循环，快捷切换 1/2/3/5/10 轮会自动换算总场次。';
}

function getFixedPairInvalidHint(validation) {
...
function buildFixedPairReadinessHint(validation) {
  const validTeamsCount = Math.max(0, Number(validation && validation.validTeamsCount) || 0);
  if (validation && validation.hasInvalid) {
    return `${getFixedPairInvalidHint(validation)}（当前合法队伍 ${validTeamsCount}）`;
  }
  if (validTeamsCount >= 2) return `已组 ${validTeamsCount} 支队伍`;
  return `需至少2支合法队伍（当前${validTeamsCount}）`;
}
...
module.exports = {
  ...
  buildFixedPairCycleShortcutOptions,
  buildFixedPairShortcutHint,
  getFixedPairInvalidHint,
  buildFixedPairReadinessHint
};
