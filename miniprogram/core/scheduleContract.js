...
function resolveDisplayTotalMatches(tournament, fallbackTotal = 0) {
  const scheduledMatches = Math.max(0, Math.floor(Number(tournament && tournament.scheduledMatches) || 0));
  if (scheduledMatches > 0) return scheduledMatches;
  const configuredMatches = Math.max(0, Math.floor(Number(tournament && tournament.totalMatches) || 0));
  if (configuredMatches > 0) return configuredMatches;
