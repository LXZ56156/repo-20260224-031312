...
test('governance separates current status, product boundary and archived restart evidence', () => {
  ...
  assert.equal(registry.worktrees.filter((item) => item.lifecycle === 'archive_pending').length, 11);
  assert.equal(registry.worktrees.filter((item) => item.lifecycle === 'archived').length, 5);
