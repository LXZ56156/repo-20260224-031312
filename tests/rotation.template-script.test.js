...
test('rotation template generator is deterministic for a coverage milestone case', () => {
  ...
  execFileSync('node', [script, '--cases', '8p-2c', '--output', first, '--frontend-output', firstMatchOptions], { cwd, stdio: 'pipe' });
  execFileSync('node', [script, '--cases', '8p-2c', '--output', second, '--frontend-output', secondMatchOptions], { cwd, stdio: 'pipe' });
