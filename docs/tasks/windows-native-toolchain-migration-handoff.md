...
- The Windows-native tooling remains the accepted implementation and is being ported onto `codex/ui-optimization-v2`, which starts from `master@5813ffc`.
...
- The 2026-07-14 cold-start/final review fixed four additional non-functional gaps: empty process collections after CLI quit, a premature second `auto` that could race a slow AppService compile, unstable DevTools top-level window selection/foreground restoration, and test-file CPU contention against wall-clock squad-search budgets. A single-auto cold start completed in `65.524s`; the final test runner serializes files without changing the production scheduler or fairness thresholds.
...
- Final current-branch verification passed 1164 tests (1158 passed, 6 skipped, 0 failed), deprecated API and cloud-common checks, lint with 0 errors, and `git diff --check`.
...
- Local `miniprogram-automator` is pinned to `0.12.1`. Enhanced screenshot smoke succeeded for `launch`, `scheduleRunning`, and `home` from `D:\projects(WIN)\badminton-miniapp`; the latest schema-v3/focus-bound record took `60.521s`. Each case proves a visible fixture transition plus two stable restored frames; promotion is transactional across the requested set, the exact session window must be restored from/to minimized `showCmd=2` with the current user foreground restored before success, fake fixture sync errors must remain zero, and only smoke/full sets update the canonical record.
...
- Current product progress and online-version assumption: `docs/tasks/current.md`
