...
- 约束：不部署、不上传；保留 `create` / `analytics` 旧路由兼容；Windows 本机 `.git/hooks/post-commit` 已安装并通过 skip 验证，但不得用它触发云函数部署
...
Windows 迁移结果：日常开发项目为 `D:\projects\badminton-miniapp`，主 launcher 为 `D:\weapp-mcp-launcher\weapp-main-dev.cmd`，自动化端口 `ws://127.0.0.1:39420`。旧 `D:\weapp-mcp-launcher\weapp-mcp.cmd` + `D:\projects\badminton-miniapp-preview` 仅保留给 preview/upload 镜像用途。Codex hooks 已改为跨平台 Node wrapper：Windows 调主项目 preflight，非 Windows 默认 no-op，只有 `WEAPP_CODEX_DEV_MODE=wsl-mirror` 才启用旧镜像链路；空 prompt / 非小程序 prompt 会跳过，避免 Codex 启动时自动打开 DevTools。
