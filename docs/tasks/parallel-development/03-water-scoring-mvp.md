...
| 审批项 | 建议方案 | 影响页面 | 待用户确认 |
|---|---|---|---|
| 功能入口 | 赛事设置中增加「打水记账」开关 | settings | 是 |
| 默认行为 | 新建和历史赛事均默认关闭；关闭时不显示额外录分控件 | settings、match、analytics | 是 |
| 模式范围 | 仅 `multi_rotate` 可开启；其他模式不展示、不接受写入 | settings、match | 是 |
| 单局瓶数 | 开启后每局选择负方每人 `0 / 1 / 2` 瓶；建议默认 `1` 瓶 | match | 是 |
| 记账定义 | 每名负方 `请水 += N`，每名胜方 `赢水 += N`，双方总量相等 | match、analytics | 是 |
| 榜单位置 | analytics 增加独立「打水榜」，展示赢水、请水、净水 | analytics | 是 |
| 正式排名 | 打水数据永不进入 `rankings`，不改变胜场、分差或名次 | 无排名 UI 变化 | 是 |
| 改分语义 | 改比分或改瓶数时按当前单局快照重算；不做增量累加 | match、analytics | 是 |
| 导航与分享 | MVP 不增加页面、导航、分享文案或分享卡片字段 | 无 | 是 |
